...
authCommand.command("login").description("登录 Round2AI 账户").action(async () => {
...
// 数据对接命令
program.command("ingest").description("对接主流 ERP，统一多渠道经营数据").action(() => {
...
program.command("ask").description("自然语言查询经营数据").action(async () => {
...
program.command("bidding-strategy").description("基于预算与品类生成竞价策略建议").action(() => {
...
pricingCommand.option("--sku <sku>", "商品SKU").option("--condition <condition>", "商品成色").action((options) => {
...
program.command("sell-optimize").description("在售商品的定价与渠道优化建议").action(() => {
...
inventoryCommand.command("risk").description("库存风险识别").option("--warehouse <warehouse>", "仓库代码").action((options) => {
...
program.command("fulfillment").description("履约全链路追踪").action(() => {
